...
- **Contrast is verified against the surface the component actually paints on**, not against
  ...
  measured ratios per token: white / `surface-alt` / own 10% tint. **Those recorded numbers are
  not all trustworthy:** `--color-brand-orange-text` (`#c24a00`) is documented as 5.98 / 5.56 /
  5.32 and actually measures 4.91 / 4.57 / 4.50 — it clears AA by 0.07 and fails on the two brand
  surfaces the system hasn't adopted yet (the manual's `#F2F4F7` grey at 4.46 and the sand at
  4.01). Recompute before trusting a comment; see `docs/brand/sistema-de-identidad.md` §4.5.3.
...
- **[`docs/brand/sistema-de-identidad.md`](docs/brand/sistema-de-identidad.md)** ★ — **The brand's source of truth, and the only brand doc you need to read end to end.** It reconciles the client's two official sources — the **manual vivo** (the live visual system: the *real* logo, a geometric wordmark with "Bro" in navy, "Way" in orange and a dove symbol; archived at [`docs/brand/manual-vivo/`](docs/brand/manual-vivo/)) and the **manual de marca v2.0** PDF (strategy, verbal identity, governance) — and resolves the nine contradictions between them with an explicit verdict and precedence *by subject matter*. It holds the 12 inviolable principles, the official palette with a recomputed contrast matrix, and the repo's identity debt with file and line. **Two consequences that touch code directly:** the brand hex values are `#0D3B66` / `#16B4C6` / `#FF8A00` / `#F6E7C3` / `#F2F4F7`, so the ones in `app/globals.css` today (`#003062` / `#00aac3` / `#ff6a03`) are wrong — they were sampled from a picture of the logo instead of read from the palette; and the logo the site ships (`/logo-broway.png`, the old cursive signature) is no longer the company's logo. Do not "fix" either one inline while working on something else: both change rendered pages and measured contrast, so they belong in their own step (§6 lists them). The seven open brand decisions were closed on 2026-09-11 and live in [`docs/brand/aprobaciones.md`](docs/brand/aprobaciones.md) — **a decision that is not in that file is not a decision**. The one that touches code hardest: the typeface is now **Manrope**, a single family, and Caveat leaves the system.
- **[`docs/brand/manual-de-marca.pdf`](docs/brand/manual-de-marca.pdf)** — Official brand manual v2.0, 27 pages: name rules, brand DNA (archetypes, values, positioning), voice/tone, the "Next Stop" verbal platform (a narrative/audiovisual signature — never merged into the logo, never used more than once per piece), and — contrary to what this file said until 2026-09-10 — it **does** define color hex values with RGB and CMYK (§15), the three typefaces with their scale and restrictions (§18-19), a verified contrast table (§17) and photography direction (§20). What *is* superseded is its **logo system** (§12-14, the cursive signature with plane and road), replaced by the manual vivo. Read it when `sistema-de-identidad.md` cites a section and you want the original; never use it as the logo source. Previous logo file: [`docs/brand/logo-broway-adventures.png`](docs/brand/logo-broway-adventures.png).
